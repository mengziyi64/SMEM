...
switch init
    case 0   % initialize at zero, using AT to find the size of x
       x = AT(zeros(size(y)),'single');
    case 1   % initialize randomly, using AT to find the size of x
       x = randn(size(AT( zeros(size(y),'single') )));
...
%     if mod(iter,50) == 0 && verbose
%         figure(999);imagesc(plotdatacube(x));colormap gray;axis image;colorbar;title(iter);set(gcf,'color','w');drawnow;
%     end
    %disp(num2str(iter));
    if exist('handles','var')
        set(handles.it_num,'String',num2str(iter)); pause(.0001);
        if iter > 3 %iter==7
            set(handles.est_time,'String',num2str( (cputime-t1)*maxiter/iter) ); pause(.0001);
        end
    end
end
%--------------------------------------------------------------
