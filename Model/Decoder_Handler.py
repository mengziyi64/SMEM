...
class Decoder_Handler(Basement_Handler):
    ...
    def train(self):
        ...
        while epoch_cnt <= self.epochs:
            ...
            for trained_batch in range(0,self.train_size):
                ...
                if trained_batch%5 == 0 and trained_batch != 0:
                    Train_loss = np.mean(np.asarray(Tresults["loss"][-5:]))
                    Train_psnr = np.mean(np.asarray(Tresults["psnr"][-5:]))
            ...
            if Vpsnr >= max_val_psnr:
                matcontent = {}
                matcontent[u'truth'],matcontent[u'pred'] = list_truth, list_pred
                sio.savemat('./' + self.log_dir + 'Validation_result.mat', matcontent)
                #hdf5storage.write(matcontent, './', self.log_dir+'Validation_result.mat',store_python_metadata=False, matlab_compatible=True)
                max_val_psnr = Vpsnr
    ...
    def test(self):
        ...
        for i in range(len(self.testing_list)):
            sio.savemat('./'+self.log_dir+'/Recon_'+self.testing_list[i], {'recon': np.squeeze(test_output['pred_orig'][i,:,:,:])} )
